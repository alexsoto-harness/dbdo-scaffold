...
6. **A GitHub repo** containing a `liquibase/changelog.yaml` file (see [Changelog Repo](#changelog-repo))
...
The workshop uses a separate GitHub repo to store Liquibase changelogs. This repo must contain a `liquibase/changelog.yaml` file. A starter file is provided in `dbdo-sample-init/changelog.yaml` — push it to your changelog repo before running Terraform.
...
| `github_repo` | `dbdo-sample` | GitHub repo containing the changelogs/migrations (see note below) |
